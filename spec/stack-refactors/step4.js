describe("stack", function() {
  ...
  it('should have "add", "remove", and "size" methods', function() {
    ...
    expect(stack.toString).toEqual(jasmine.any(Function));
  });
  ...
  it('should be capable of mutation', function(){
    Stack.prototype.testFunction = 3;
    expect(stack.testFunction).toEqual(3);
  });
  ...
  describe('add', function() {
    ...
    it('three items and remove one, index of first item should remain the same', function(){
      ...
      stack.add('parakeet');
      stack.remove();
      expect(stack.storage[0]).toEqual('cat');
      expect(stack.storage[1]).toEqual('dog');
    });
  });
